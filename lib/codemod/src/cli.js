...
const TRANSFORMS = {
  ...
  'update-linkinline': 'Replaces deprecated Link component with Linkinline',
  'update-linkstandalone':
    'Replaces deprecated Link component with LinkStandalone',
...
async function runTransform(codemodName, files, args) {
  const transformPath = path.join(__dirname, `${codemodName}.js`);
...
yargs
  ...
  .usage(
    ...
    Example:
      $ ${PACKAGE_NAME} v6.0.0/update-tokens ./src/**/*.+(ts|tsx|json) 
