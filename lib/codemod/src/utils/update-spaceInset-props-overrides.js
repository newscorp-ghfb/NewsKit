...
const {getPadding} = require('./replace-space-inset');
...
const updateProps = (root, j, source, componentName, propFn) => {
  return root.findJSXElements(componentName).forEach(path => {
    ...
    attributes.forEach((node, index) => {
      if(node.type === 'JSXAttribute' && node.name.name === 'spaceInset' && node.value.type==='Literal') {
          const propValue = node.value.value;
          const {paddingBlock, paddingInline} = propFn(propValue); 
          
          attributes[index] = j.jsxAttribute(j.jsxIdentifier('paddingBlock'), j.literal(paddingBlock))
          attributes[index + 1] = j.jsxAttribute(j.jsxIdentifier('paddingInline'), j.literal(paddingInline))
      }
      if(node.type === 'JSXAttribute' && node.name.name === 'spaceInset' && node.value.type==='JSXExpressionContainer') {
        ...
        attributes[index] = j.jsxAttribute(j.jsxIdentifier('paddingBlock'), j.jsxExpressionContainer(objectToExpression(j, paddingBlock)))
        attributes[index + 1] = j.jsxAttribute(j.jsxIdentifier('paddingInline'), j.jsxExpressionContainer(objectToExpression(j, paddingInline)))
      }
    })
  })
};
