...
const getPadding = token => {
  if (typeof token === 'string' && token.match(/\w+\s\w+/gm)) {
    const tokensArray = token.split(' ');
    return {paddingBlock: tokensArray[0], paddingInline: tokensArray[1]};
  }
  ...
  if (typeof token === 'object') {
    ...
    return mqKeys.reduce(
      (obj, mqKey) => {
        ...
        const {paddingBlock, paddingInline} = tokenValue.includes('px')
          ? {paddingBlock: tokenValue, paddingInline: tokenValue}
          : map[tokenValue];
        return {
