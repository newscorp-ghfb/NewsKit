...
const addSimpleProp = (path, prop, propValue, jscodeshift, root) => {
  const existing = getAttributeValue(path, prop);
  if (!!existing && typeof propValue === 'function') {
    const obj = getChunk(
      root.toSource(),
      existing.value.expression.start,
      existing.value.expression.end,
    );
...
module.exports = function addProp(
  ...
  return root.findJSXElements(componentName).forEach(path => {
    if (!prop.includes('.')) {
      addSimpleProp(path, prop, propValue, jscodeshift, root);
    } else {
...
const getChunk = (file, start, end) => {
  const obj = eval(`(${file.substring(start, end)})`);
  return obj;
};
