...
const updateSpaceInsetProps = (root, j, source, componentName, propFn) =>
...
const updateOverrides = (root, j, source, componentName, propName, propFn) => {
  root.findJSXElements(componentName).forEach(path => {
    const attributes = path.node.openingElement.attributes;
