...
NewsKit offers you the ability to theme foundations such as
[colour](/foundations/colours), [spacing](/foundations/spacing),
[typography](/foundations/typography), etc.
The foundations are used globally throughout the site to populate components that align with your brand identity.
...
- [Tag](/components/tag) is one of the components exported by NewsKit. In the example, the Tag component receieves two arguments:
  ...
  - `$size` - NewsKit uses a convention for props that are style-related. They should start with `$` sign to be easily recognized. The supported sizes for the tag component are placed inside the `TagSize` enum object.
