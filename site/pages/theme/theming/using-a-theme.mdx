...
In order to enable that functionality to your project, you need to add a prop `exposeCssVariables` to ThemeProvider, or pass it as themeOptions to NewsKitProvider as the example below.
NewsKitProvider themeOptions is the best option for globally turning on options. It can also be used to turn on useThemeCache, which is recommended as it significantly speeds up component rendering.

<Code>
{`<NewsKitProvider theme={yourTheme} themeOptions={{exposeCssVariables: true, useThemeCache: true}}>
  Main theme context
