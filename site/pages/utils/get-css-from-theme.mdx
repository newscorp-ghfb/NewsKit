...
The following functions `getColorCssFromTheme`, `getSizingCssFromTheme`, `getBorderCssFromTheme`, `getShadowFromTheme`, `getCssMotionFromTheme` share the same annotation, therefore we will generalize them as `the getter function`.
Also, since those functions are used in the same way, we will use only `getCssColorFromTheme` to build our Syntax and Example sections.
...
<Code>
  {'getColorCssFromTheme(cssProp, defaultToken?)(props)'}
</Code>
