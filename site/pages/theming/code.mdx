...
The `useTheme` hook similarly to the `withTheme` allows a developer to access the global theme.

<CodeFromFile path="examples/theming/use-theme.tsx"/>

## Theme

Themes represent the brand's identity. These are objects that house the foundations and tokens used inside the NewsKit components. NewsKit provides 
