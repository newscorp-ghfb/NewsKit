...
<CodeFromFile path="examples/theming/override-default-presets-in-theme.tsx" />

In the example above the `headline.heading.stylePreset` will be overridden with `buttonSolidPrimary`. Leaving the other presets untouched. Following this example the `typographyPresets` and the other fields can overridden in a similar way.
