...
Let's assume we have the following structure for the `articleHeadline` default presets.

<CodeFromFile path="examples/theming/override-default-presets.tsx" />

In the example above the `articleHeadline.heading.stylePreset` will be overridden with `buttonSolidPrimary`. Leaving the other presets untouched. Following this example the `typePrests` and the other fields can overridden in a similar way.
...
The  style preset `buttonTestStyle` is added to the theme when it is created and it can be used to override the `articleHeadline.heading.stylePreset`.  
