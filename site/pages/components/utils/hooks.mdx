...
<InlineMessage aria-label="Requirements">
  Media Query hooks require <strong>NewsKitProvider</strong> or{' '}
...
>
  Media Query hooks work only on client side code ( Browser ).
  <br />
  During server side rendering hooks will always return <strong>XS</strong>.
</InlineMessage>
