import Layout from '../../components/layout';
import Playground from '../../components/playground';
import {Prop} from '../../components/prop';
import {LegacyBlock} from '../../components/legacy-block';
import {TextBoxSwatch} from '../../components/theming-values/colors';
import {CodeFromFile} from '../../components/code';

import {TextBlock} from 'newskit';

export default Layout;

# TextBlock

## Overview

TextBlock is a paragraph, which can be used to summarize any text content. It can be in larger or bolder type than regular paragraph text and this is typically to attract the user’s attention to read more of the article.

### Usage

## Props

<LegacyBlock marginTop="sizing040">
  <Prop name="children" type="string" required>
    The content of the TextBlock is passed as the child of the component.
  </Prop>
  <Prop name="renderAs" type="'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' | 'div'" default="p">
    This is a string name with one of the following html elements: h1, h2, h3, h4, h5, h6 or div, that the textblock can be converted to.
    TextBlock defaults as a p element, but it can be converted to another heading element or div by `renderAs="div"` prop.
  </Prop>
  <Prop name="overrides" type="object">
    If provided, overrides the respective presets for the component and provided elements.
  </Prop>
</LegacyBlock>

## Playground

<Playground
  componentName="TextBlock"
  newskitPath="/text-block"
  component={TextBlock}
  knobs={[
    {
      name: 'Content',
      propName: 'children',
      value: 'TextBlock Content',
    },
    {
      name: 'Render Styled Text as',
      propName: 'renderAs',
      options: [
        {
          label: 'h1',
          value: 'h1',
        },
        {
          label: 'h2',
          value: undefined,
          isDefault: true,
        },
        {
          label: 'h3',
          value: 'h3',
        },
        {
          label: 'h4',
          value: 'h4',
        },
        {
          label: 'h5',
          value: 'h5',
        },
        {
          label: 'div',
          value: 'div',
        },
      ],
    },
  ]}
/>
