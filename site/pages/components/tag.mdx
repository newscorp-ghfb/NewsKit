import Layout from '../../components/layout';
import Playground from '../../components/playground';
import {Prop} from '../../components/prop';
import {TextBoxSwatch} from '../../components/theming-values/colors';
import {LegacyBlock} from '../../components/legacy-block';
import {CodeFromDefaultPresets} from '../../components/code';
import {Tag, TagSize} from 'newskit';

export default Layout;

# Tag

## Overview

The tag element is used within the [tag list](/components/tag-list) as a way of classifying content, typically using keywords. They are added to a web page or other content to help users search for and find related content quickly and easily.

### Usage

A tag is used in a [tag list](/components/tag-list) and displayed anywhere that a quick link to related content could be beneficial to a user e.g. article pages or search.

## Props

Tags have a number of props to facilitate a variety of uses:

<LegacyBlock marginTop="sizing040">
  <Prop name="children" type="string" required>
    The content of the Tag is passed as the child of the component.
  </Prop>
  <Prop name="href" type="string">
    If provided, the Tag will render as an anchor with this prop value as the
    href.
  </Prop>
  <Prop name="size" type="TagSize" default="Medium">
    Optional size of the Tag: Small, Medium or Large.
  </Prop>

  <Prop name="overrides" type="object">
    If provided, overrides the respective presets for the component and provided elements.
    Here are the overrides that the Tag accepts:
    <Prop name="padding" type="PaddingPresetKeys">
      If provided, this overrides the padding of the Tag. Otherwise,
      spaceInset010Squish or spaceInset020Squish is used depending on the `size`
      prop.
    </Prop>
    <Prop name="stylePreset" type="string" default="tagDefault">
      If provided, this overrides the style preset applied to the Tag.
    </Prop>
    <Prop name="typePreset" type="string">
      If provided, this overrides the type preset applied to the Tag. Otherwise,
      label010, label020 or label030 is used depending on the `size` prop.
    </Prop>
    <Prop name="space" type="SizingKeys">
      If provided, this overrides the space between multiple children in the
      underlying Stack. If less than 2 children are passed, this prop is
      irrelevant.
    </Prop>
    <Prop name="iconSize" type="IconSizeKeys" default="iconSize010">
      If provided, this overrides the icon size of any child icons. If no icons
      are present in the Tag's children then this prop is irrelevant.
    </Prop>
    <Prop name="width" type="SizingKeys | string">
      If provided, this sets a fixed width to the Tag. This can be a sizing token
      from the theme, or any CSS length value, e.g. 100% for a full-width element.
    </Prop>
    <Prop name="height" type="SizingKeys | string">
      If provided, this sets a fixed height to the Tag. This can be a sizing token
      from the theme, or any CSS length value.
    </Prop>
    <Prop name="minHeight" type="SizingKeys | string">
      If provided, this sets a minimum height to the Tag. This can be a sizing
      token from the theme, or any CSS length value. By default, sizing050,
      sizing060 or sizing070 is used depending on the `size` prop.
    </Prop>
    Refer to the defaults below for the object structure:
    <CodeFromDefaultPresets componentName="tag" />
  </Prop>
  <Prop name="disabled" type="boolean" default={false}>
    If true, the Tag will render the disabled state of the style preset. If an
    href is also passed, the Tag will not open the provided href on click.
  </Prop>
  <Prop name="isLoading" type="boolean" default="false">
    If true, this will cause the Tag to render the loading state of the style
    preset.
  </Prop>
</LegacyBlock>

## Playground

<Playground
  componentName="Tag"
  newskitPath="/tag"
  component={Tag}
  knobs={[
    {
      name: 'Content',
      propName: 'children',
      value: 'Tag Content',
    },
    {
      name: 'Link',
      propName: 'href',
      value: 'http://example.com',
    },
    {
      name: 'Disabled',
      propName: 'disabled',
      type: 'boolean',
      value: false,
    },
    {
      name: 'Tag Size',
      propName: 'size',
      options: [
        {
          label: 'Small',
          value: TagSize.Small,
        },
        {
          label: 'Medium',
          isDefault: true,
          value: TagSize.Medium,
        },
        {
          label: 'Large',
          value: TagSize.Large,
        },
      ],
    },
    {
      name: 'Style Preset',
      propName: 'overrides',
      options: [
        {
          label: 'Default',
          value: {
            stylePreset: undefined,
          },
          isDefault: true,
        },
        {
          label: 'tagPrimary',
          value: {
            stylePreset: 'tagPrimary'
          },
        }
      ],
    },
    {
      name: 'Type Preset',
      propName: 'overrides',
      options: [
        {
          label: 'Default',
          value: {
            typePreset: undefined,
          },
          isDefault: true,
        },
        {
          label: 'label010',
          value: {
            typePreset: 'label010'
          },
        },
         {
          label: 'label020',
          value: {
            typePreset: 'label020'
          },
        },
        {
        label: 'label030',
        value: {
          typePreset: 'label030'
        },
      }
      ],
    },
  ]}
/>

For more information on how to override default presets, see docs [overrides default presets](/theming/default-presets/override-individual-components).

## Rationale

### Styling

Tags are designed to be compact and discreet as they are intended to supplement the primary content.

Tags should have as few words as possible and always have a direct relationship to the content they represent.

### Accessibility

#### Colour

Text and background colours must have a 3:1 minimum contrast ratio.
Hover states should have clear contrast change or should include a non colour designator.

### SEO

#### Ranking

Tagging generates hub pages which act as valuable catch-all pages, these rank well in search, are automatically updated and generate more backlinks.

#### Navigation

Good for site navigation (for crawlers and users), particularly helpful where site structure is poor (The Times)

#### Descriptive

Tagging helps tell Google what the page content is about, this then enables Google to better surface those pages in search
