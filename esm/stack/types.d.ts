...
export type Flow = 'vertical-left' | 'vertical-center' | 'vertical-right' | 'vertical-stretch' | 'horizontal-top' | 'horizontal-center' | 'horizontal-bottom' | 'horizontal-stretch';
export type StackDistribution = 'flex-start' | 'flex-end' | 'center' | 'space-around' | 'space-between' | 'space-evenly';
...
export interface ChildProps extends Pick<StyledStackProps, 'spaceInline' | 'spaceStack' | 'flow' | '$wrap'> {
    order?: MQ<number>;
}
export interface StyledChildProps extends Pick<StyledStackProps, 'spaceInline' | 'spaceStack' | 'flow' | '$wrap' | 'as'> {
    $order?: MQ<number>;
    $alignSelf?: MQ<string>;
    flexGrow?: boolean | number;
}
